...
const promptFor = (asset: AssetEntry): string => {
  ...
  if (asset.id === 'capture-dramatic-spritesheet') return `${styleLock} Create exactly five equal square cells in one horizontal row. Cell 1: an original disguised criminal running right, pose A, wearing a black brimmed hat and long black trench coat, face hidden except the eyes. Cell 2: the same criminal running right, pose B, identical clothing and scale. Cell 3: one standalone fictional Brazilian federal agent in a blue period uniform clearly facing and running right, pose A with one leg extended. Cell 4: the same right-facing agent, pose B with the forward knee visibly bent and rear leg folded. Cell 5: a complete escort pair facing and walking left, with the captured criminal's hands raised and an agent immediately behind guiding the prisoner with a safely carried shoulder or holstered weapon. Stable full-body scale and foot anchor, no overlap between cells. Flat chroma green #00FF00 background. ${negatives}`;
